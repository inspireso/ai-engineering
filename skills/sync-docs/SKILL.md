...
version: 1.0.0
description: >
  会话结束后对项目文档和记忆进行洁癖级审查与同步——确保 CLAUDE.md、README.md、
  shared/AGENTS.md、docs/ 和 agent 记忆保持一致、精简、准确，知识体系不腐烂。
  合并了 neat-freak 的全部能力（尺寸体检、记忆毕业、变更影响矩阵、跨项目对齐）。
allowed-tools:
  - Read
  - Edit
  - Write
  - Bash(git:*,ls,find,wc,du,grep)
  - Grep
  - Glob
  - Agent
  - TaskCreate
...
> 合并 neat-freak 全部能力：项目文档同步 + 记忆毕业 + 尺寸体检 + 跨项目影响对齐。
...
这三层**受众不同，职责不重叠**。CLAUDE.md 里写"新增了 X 功能" ≠ docs/ 里"怎么用 X" —— 前者是提醒自己，后者是教别人。**两份都要写。**
...
必须理解这条不对称，否则记忆永远在膨胀：**docs 靠就地编辑收敛**（系统改 10 次，还是那一份架构文档），**而 agent 记忆天生只追加**（每条教训生一个新文件，旧的不删）。没有反向阀门，memory 会一路堆到比 docs 还大，真正稳定的知识被困在几十个松散文件里——既进不了 prompt（索引超 25KB 截断），也没沉淀成给别人看的文档。
...
- **它是「X 上线 / 落地 / 就位」的事件记录** → 现役事实进 docs，过程进 git log / CHANGELOG，memory 不留常驻文件。
...
最常见的 skill 翻车模式：每次开发完都在 CLAUDE.md 顶部加一段 blockquote 历史叙事。一次很爽，半年后顶部就是 200 行 blockquote 把真正的规则推到看不见。**这种叙事不属于 CLAUDE.md**，它的归宿是 git log / CHANGELOG / `docs/CHANGES.md`。
...
| 文件 | 上限 | 超过怎么办 |
|---|---|---|
| `CLAUDE.md` / `AGENTS.md` | ~300 行 / ~15KB（软上限） | 先精简：扫顶部 blockquote / 历史叙事段 → 删 / 迁 docs；项目概览只留 1-3 行 + 速查表。CLAUDE.md 全量加载但越长 adherence 越差 |
| 记忆索引 `MEMORY.md` | **≤200 行 且 ≤25KB（硬上限）** | Claude Code 只加载前 200 行或前 25KB（先到先算），**超出部分在会话开始时静默不加载——等于没记**。务必压在 ~150 行 / ~18KB 留缓冲。压法不是硬删，是毕业机制：详细机制提升进 docs、索引只留一行指针 |
| 单条 memory 文件 | ~100 行（软上限） | 通常在塞多件事 / 写成事故复盘 → 拆 / 删；**若是稳定机制说明，提升进 docs 再把记忆缩成 reference 指针** |
| `docs/<single>.md` | ~1500 行（软上限） | 切分成多文件，加目录索引 |

**额外做「体量倒挂」体检**：对比 memory 目录和 docs/ 的体积。**健康态是 docs 厚、memory 薄**——docs 是沉淀的权威层，memory 是流动的「最近教训 + 指针」层。若 memory 反而比 docs 大，几乎一定是「本该毕业进 docs 的稳定知识还赖在松散记忆文件里」，按「毕业」机制往上泵，别只在 memory 内部挪。
...
1. 列出 agent 的记忆文件（如有）：
   - Claude Code：列出 `memory/` 并读 `MEMORY.md` 及所有被引用的 `.md`
   - Codex / OpenCode / 其他：找该 agent 的等价位置（见 references/agent-paths.md）
2. 对本次对话涉及的**每一个项目**：
   - 确认根目录结构
   - 枚举所有 docs（缺失也要确认）
   - 枚举 skills/ 下每个子目录的 `SKILL.md` 和 `references/*.md`（嵌套在子目录中，浅层 find 会漏掉）
   - 兜底抓散落的 .md（排除 node_modules、.git，使用足够深的层级覆盖嵌套目录）
   - 读 `README.md`、`CLAUDE.md` / `AGENTS.md`、每一个 `docs/*.md`
3. 读全局 agent 配置（若有，如 `~/.claude/CLAUDE.md`、`~/.codex/AGENTS.md`）
4. 回顾本次对话全部内容

**输出一张内部文件清单**，对每个文件标：「评估过 / 要改 / 不用改」。**漏一个不行**——这是这个 skill 最容易翻车的地方。
...
**插件 / 规范库项目**（skill、command、hook、配置）：
- 新增 skill → README skills 表格 + 目录结构 + 必要时 plugin.json
- 新增 command → README commands 表格 + 目录结构
- 新增 hook → README hooks 表格 + 目录结构 + hooks.json（如适用）
- 新增 reference / 设计文档 → docs/ 索引 + README 必要时更新
- 修改插件配置（settings.json / plugin.json）→ README 安装/配置章节
...
**顺序建议**：先改 docs/（改错影响外部）→ 再改 README → 再改 CLAUDE.md/AGENTS.md → 最后理记忆。先动外部优先级最高的，即使中途被打断，读者看到的也是对齐的最新状态。
...
- **毕业优于内部挪腾**（针对 memory）：一条记忆稳定、复用、或本属「系统怎么工作」时，别在 memory 里搬来搬去——并进 docs / CLAUDE.md，原文件缩成一行指针或删。这是把 memory 压回「薄」的唯一治本手段
...
API 速查表、环境变量表、术语表、README 的 skills/commands/hooks 表格是高频查询的结构化信息，**必须保持「所见即最新」**。
...
- [ ] **记忆索引 `MEMORY.md` ≤ 25KB 且 ≤ 200 行**（实测字节数；超出部分会话开始时静默不加载 = 等于没记）
- [ ] **体量没倒挂**：memory 不应大于 docs/；倒挂了说明有该毕业进 docs 的知识赖在 memory，回去毕业
...
- [ ] README 的安装 / 运行步骤跟代码一致、结构化表格（skills/commands/hooks）与文件系统一致
...
- [ ] 没有相对时间遗留（「今天」「昨天」「刚刚」「最近」「上周」「today」「yesterday」「recently」清零）
